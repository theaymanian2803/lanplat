...
const Index = () => {
  ...
  const { data: videos = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filter !== "all") q = q.eq("language", filter as Language);
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <ToggleGroup type="single" value={filter} onValueChange={(v) => v && setFilter(v)} className="justify-start">
          ...
          {LANGUAGES.map((l) => (
            <ToggleGroupItem key={l} value={l} className="font-mono text-xs">{l}</ToggleGroupItem>
          ))}
        </ToggleGroup>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {videos.map((v) => {
              ...
              return (
                <Card key={v.id} className="group overflow-hidden border-border/50 hover:border-primary/30 transition-colors">
                  <Link to={`/video/${v.id}`}>
                  ...
                  <CardContent className="p-3 flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <Link to={`/video/${v.id}`} className="font-medium text-sm truncate block hover:text-primary transition-colors">
                        {v.title}
                      </Link>
                      <Badge variant="outline" className={`mt-1.5 text-[10px] font-mono ${langColors[v.language]}`}>
                        {v.language}
                      </Badge>
                    </div>
                    <Button
                      ...
                      onClick={(e) => { e.preventDefault(); deleteVideo.mutate(v.id); }}
                    >
        ...
        <DialogContent>
          ...
          <DialogFooter>
            <Button
              ...
              disabled={!newUrl || !newTitle || addVideo.isPending}
